...
　　Hadoop生态和其他生态最大的不同之一就是“单一平台多种应用”的理念了。传的数据库底层只有一个引擎，只处理关系型应用，所以是“单一平台单一应用”；而NoSQL市场有上百个NoSQL软件(比如cassandra,mongoDB等)，每一个都针对不同的应用场景且完全独立，因此是“多平台多应用”的模式。而Hadoop在底层共用一份HDFS存储（现在存储层也可以用HBase，Kudu来做替换，但管理层是统一的，所以对计算引擎层是透明的。），上层有很多个组件分别服务多种应用场景，如：
